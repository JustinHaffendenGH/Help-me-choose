...
async function getRandomCuratedFood() {
  ...
  const availableFoods = curatedFoods.filter(
    (food) => !foodCache.lastShownFoods.includes(food.name)
  );
...
async function getRandomGooglePlacesFood() {
  ...
  try {
    ...
    if (data.results && data.results.length > 0) {
      ...
      const goodRestaurants = data.results.filter(
        (place) =>
          place.rating >= 4.0 &&
          place.photos &&
          place.photos.length > 0 &&
      ...
      if (goodRestaurants.length > 0) {
        const randomRestaurant =
          goodRestaurants[Math.floor(Math.random() * goodRestaurants.length)];
...
async function showRandomFood() {
  ...
  const promises = [getRandomGooglePlacesFood(), getRandomOpenFoodFactsFood()];
  ...
  try {
    ...
    if (validFoods.length > 0) {
      const bestFood =
        validFoods.find((food) => food.source === 'googleplaces') ||
...
function showFoodLoadingState() {
  ...
  const locationText = userLocation.isSet
...
async function displayFoodWithLoadingSequence(food) {
  ...
  if (food.cuisine) {
    foodData.cuisine =
      food.cuisine.charAt(0).toUpperCase() + food.cuisine.slice(1);
  ...
  try {
    ...
    if (curatedFoodsFiltered.length > 0) {
      const randomIndex = Math.floor(
        Math.random() * curatedFoodsFiltered.length
      );
...
async function getFilteredGooglePlacesFoods() {
  ...
  try {
    const response = await fetch(
      `/api/places/search?query=${query}+restaurant&location=${getLocationString()}&type=restaurant`
    );
    const data = await response.json();

    if (data.results) {
      const filteredFoods = data.results
        .filter((place) => {
...
async function getFilteredOpenFoodFactsFoods() {
  ...
  try {
    ...
    if (data.products) {
      const filteredFoods = data.products
        .filter((product) => {
          const rating = product.nutriscore_grade
            ? { a: 5, b: 4, c: 3, d: 2, e: 1 }[
...
function displayNoFoodResults() {
  ...
  if (foodDescription)
    foodDescription.textContent =
      'Try adjusting your filters to find more foods.';
